...
def seed_routes(tx):
    routes = [
        ...
        {"name": "Russia to Vadinar via Cape", "avg_transit_days": 22, "distance_km": 17500},
...
def seed_relationships(tx):
    queries = [
        ...
        "MATCH (s:Supplier {name:'Russia'}), (r:Route {name:'Russia to Vadinar via Cape'}) MERGE (s)-[:SHIPS_VIA]->(r)",
        ...
        "MATCH (r:Route {name:'Russia to Vadinar via Cape'}), (c:Chokepoint {name:'Cape of Good Hope'}) MERGE (r)-[:PASSES_THROUGH]->(c)",
        ...
        "MATCH (r:Route {name:'Russia to Vadinar via Cape'}), (p:Port {name:'Vadinar'}) MERGE (r)-[:ARRIVES_AT]->(p)",
