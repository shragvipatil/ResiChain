...
async def run_agent6(playbook_id=None) -> dict:
    ...
    # Step 1 — determine blocked chokepoints from live risk state
    blocked_chokepoints = await _get_blocked_chokepoints()
    logger.info(f"Agent 6: Blocked chokepoints = {blocked_chokepoints}")
    ...
    candidates = await _build_candidates(surviving_routes, blocked_chokepoints)
...
async def _get_blocked_chokepoints() -> list:
    """Reads live risk:state from Redis, returns corridors at/above the survival threshold."""
    try:
        r = await get_redis()
...
async def _build_candidates(surviving_routes: list, blocked_chokepoints: list) -> list:
    """
    ...
    for route in surviving_routes:
        ...
        # Determine which chokepoint this route primarily uses,
        # by checking route name (built by neo4j_migration as "<supplier>_to_India_via_<chokepoint>")
...
def _extract_chokepoint_from_route_name(route_name: str) -> str:
    """Route names are '<supplier>_to_India_via_<chokepoint>' — pull the chokepoint back out."""
    if "_via_" in route_name:
        return route_name.split("_via_")[-1]
    return "Unknown"
