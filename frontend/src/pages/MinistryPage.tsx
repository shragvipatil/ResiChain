/**
 * MinistryPage.tsx — updated Day 5
 *
 * Added:
 *  - useAppContext() for live riskState from WebSocket (replaces one-time fetch)
 *  - AgentStatusPanel
 *  - RiskWeightSliders
 *  - ShippingMap still present from Day 3
 */
...
const MinistryPage: React.FC = () => {
  // Pull riskState from context (WebSocket keeps this live after Day 5)
  const { riskState, setRiskState, wsConnected } = useAppContext();

  // Seed initial risk state from API on first mount
  useEffect(() => {
    if (!riskState) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 p-8">
      ...
      {/* Corridor risk score cards — update live from WebSocket */}
      <div className="grid grid-cols-4 gap-4 mb-8">
        {riskState
          ? Object.entries(riskState.corridors).map(([corridor, risk]) => (
      ...
      <div className="grid grid-cols-3 gap-6 mb-6">
        ...
        <div className="col-span-2 bg-slate-800 rounded-xl border border-slate-700 p-1">
          ...
          {/*
            Day 7: add vessels={vesselData} prop — AIS markers render automatically.
            compoundDisruptionDetected from context triggers Cape animation (built Day 7).
          */}
      ...
      {/* Risk weight sliders — full width below map */}
      <div className="mb-6">
        <RiskWeightSliders />
      </div>

      {/* Crisis mode / last updated bar */}
